...
(function (d) {
	...
	Muse.Utils.allStyleSheetRulesFromOneSheet = function (c, a) {
		...
		if (!j) b = !0, j = c.rules;
		if (!j) return null;
		for (var h = [], i = d(window), k = function (a) {
	...
	Muse.Utils.getCanvasDirection = function (c, a) {
		var b = c.closest("*[data-rotate]"),
			b = b.length > 0 ? parseFloat(b.data("rotate")) % 360 : 0;
		return {
			dir: b >= 0 && b <= 45 || b >= 135 && b <= 225 || b >= 315 && b < 360 ? a : a === "horizontal" ? "vertical" : "horizontal",
			reverse: a === "horizontal" ? b >= 135 && b <= 315 : b >= 45 && b <= 225
		}
	};
	Muse.Utils.urlParam = function (c, a) {
		var b = RegExp("[\\?&]" + a + "=([^&#]*)").exec(c);
		return b ? b[1] : null
	};
		...
		if (!j || j == "_self") {
			...
			if (h && j > f) {
				var b = g.data("scrollWrapper"),
					i = a.substring(j),
					k = Muse.Utils.getAnchorWithDestination(i);
				...
				var j = k.offset(),
					...
					f = (b ? b.scrollWidth() : f.scrollWidth) - g.width(),
					m = Math.min(h, j.top + (b && !b.isStandard() ? b.scrollTop() : 0)),
					n = Math.min(f, j.left + (b && !b.isStandard() ? b.scrollLeft() : 0)),
					p = d(k).parents(".AccordionPanelContentClip").length,
					q = function () {
						var a = d(k).closest(".AccordionPanelContent").prev();
						a && a.length && !a.hasClass("AccordionPanelTabOpen") && (a.trigger("mousedown"), a.trigger("mouseup"))
					};
				if (p) {
					var f = d(k).closest(".AccordionPanelContent"),
						h = g.scrollTop(),
						o = g.scrollTop() + g.height(),
						r = g.scrollLeft() + g.width(),
						g = g.scrollLeft(),
						o = j.top >= h && j.top + f.height() <= o,
						j = j.left >= g && j.left + f.width() <= r;
					if (o && j) return q(), !1;
					m = o ? h : Math.min(m, d(k).parents(".AccordionWidget").offset().top);
					n = j ? g : Math.min(n, d(k).parents(".AccordionWidget").offset().left)
				}
				g = function () {
					l.scrollTo(n, m);
					p && q();
				...
				if (window.scrollTo || void 0 !== b) {
					...
					d({
						...
						easing: a,
						step: function (a) {
							a != 0 && (x = a * (m - w), s = a * (n - v), l.scrollTo(v + s, w + x))
						},
						complete: g
					})
		...
		var a = function () {
			...
			if (a.length !== 0) {
				var j = a.offset(),
					...
					k = (h ?
						h.scrollHeight() : i.scrollHeight) - f.height(),
					f = (h ? h.scrollWidth() : i.scrollWidth) - f.width(),
					k = Math.min(k, j.top + (h && !h.isStandard() ? h.scrollTop() : 0)),
	...
	Muse.Utils.addHyperlinkAnchor = function (c) {
		...
		c.bind("mouseup keyup", function (a) {
			if (Muse.Utils.isMouseLeftClick(a) && b.indexOf(this) != -1) a.ctrlKey || a.metaKey ? Muse.Utils.redirectHyperlinkInNewTab(this.href, a.shiftKey) : Muse.Utils.redirectHyperlink(this);
				...
				if (d.browser.webkit || !a.target.href && d.browser.msie) Muse.Utils.redirectHyperlinkInNewTab(this.href,
					a.shiftKey);
				else return b = [], !0;
	...
	Muse.Utils.addHyperlinkBlock = function (c) {
		...
		a.bind("mouseup keyup", function (a) {
			Muse.Utils.isMouseLeftClick(a) && b.indexOf(this) != -1 ? a.ctrlKey || a.metaKey ? Muse.Utils.redirectHyperlinkInNewTab(c.href,
				a.shiftKey) : Muse.Utils.redirectHyperlink(c) : Muse.Utils.isMouseMiddleClick(a) && b.indexOf(this) != -1 ? Muse.Utils.redirectHyperlinkInNewTab(c.href, a.shiftKey) : Muse.Utils.isRedirectLinkKeyboardAction(a) && Muse.Utils.redirectHyperlink(c);
			return !1
		});
	...
	Muse.Utils.enableAnchorLinksActiveState = function () {
		var b = !1,
			...
			p = function () {
				...
				var w, s, p = Math.max(0, j);
				for (j = Math.min(j + 2, a.length); p < j; p++)
					if (w = a[p], q = w.$elem.offset().left + (l && !l.isStandard() ? l.scrollLeft() : 0), s = w.$elem.offset().top + (l && !l.isStandard() ?
							l.scrollTop() : 0), w.from != (b ? q : s)) {
						i(n);
						break
					}
				p = 0;
				for (j = a.length; p < j; p++) {
					...
					w = w.hyperLinks;
					s = void 0;
					for (var x = 0; x < w.length; x++) s = f(w[x]), q && !w[x].hasClass(s) ? w[x].addClass(s) : !q && w[x].hasClass(s) && w[x].removeClass(s)
				}
			},
			q = function () {
				k || (k = !0, Muse.Utils.requestAnimationFrame(p))
			};
		(l = g.data("scrollWrapper")) ? l.registerUpdateCallback(q): g.scroll(q);
	...
	Muse.Utils.saveHyperlinkInfo = function (b, a, g, j, f) {
		...
		if (h && -1 != h.indexOf("#") && !(l && l != "_self") && !(0 <= i.indexOf("/")) && (m.charAt(m.length - 1) == "/" && (m += "index.html"), -1 != m.indexOf("/" + i, m.length - i.length - 1))) {
			var i = d(window).data("scrollWrapper"),
				n = h.substring(h.lastIndexOf("#")),
				b = b.parent('[class~="sbg"]').length > 0 || b.hasClass("block") ? b.parent() : b,
				p = "#" + b.attr("id"),
				a = "." + a;
			if (null !== Muse.Utils.allStyleSheetRules(g, function (b) {
				...
				if (-1 == k) {
					...
					l = i && !i.isStandard();
					j = Math.floor(j ? g.offset().left + (l ? i.scrollLeft() : 0) : g.offset().top + (l ? i.scrollTop() : 0));
					i = Number.MAX_VALUE;
					l = 0;
					for (m = f.length; l < m; l++)
						if (f[l].href != h && f[l].from ==
							j) {
							k = l;
							break
						}
	...
	Muse.Utils.getNaturalWidth = function (b) {
		var a = -1;
		b.naturalWidth != null ? a = b.naturalWidth : b.runtimeStyle ? (b.runtimeStyle.width = "auto", b.runtimeStyle.height = "auto", b.runtimeStyle.borderWidth = "0", b.runtimeStyle.padding = "0", a = b.offsetWidth, b.runtimeStyle.width = "", b.runtimeStyle.height = "", b.runtimeStyle.borderWidth = "", b.runtimeStyle.padding = "") : (b = b.cloneNode(!0), b.className = "", b.style.width = "auto !important", b.style.height = "auto !important", b.style.borderWidth = "0 !important",
			b.style.padding = "0 !important", a = b.width);
		return a
	};
	Muse.Utils.getNaturalHeight = function (b) {
		...
		b.naturalHeight != null ? a = b.naturalHeight : b.runtimeStyle ? (b.runtimeStyle.width = "auto", b.runtimeStyle.height = "auto", b.runtimeStyle.borderWidth = "0", b.runtimeStyle.padding = "0", a = b.offsetHeight, b.runtimeStyle.width = "", b.runtimeStyle.height = "", b.runtimeStyle.borderWidth = "", b.runtimeStyle.padding = "") : (b = b.cloneNode(!0), b.className = "", b.style.width = "auto !important", b.style.height = "auto !important", b.style.borderWidth =
			"0 !important", b.style.padding = "0 !important", a = b.height);
		return a
	};
	...
	(function (b) {
		...
		j.prototype.init = function (f) {
			...
			if (0 != this.$verticalSpacer.length) {
				...
				a.resize(function () {
					if (i) {
						var b = a.width();
						k ? (j.splice(0, j.length), j.push(b), k = !1, setTimeout(function () {
							k = !0
		...
		var h = null,
			...
			k = h.css("height", "calc(100vh + 300px)").height();
		0 < i && 0 < k && 300 == k - i ? (h.remove(), h = new j) : (b("html").removeClass("css_verticalspacer"), h = new f);
		...
		if (-1 != a) {
			...
			for (var a = document.getElementsByTagName("meta"), b = 0, c = a.length; b < c; b++) {
				...
				if ("generator" == d.getAttribute("name")) {
					"2017.0.2.363" != d.getAttribute("content") && Muse.assets.outOfDate.push("museutils.js");
					break
				}
			}
		}
