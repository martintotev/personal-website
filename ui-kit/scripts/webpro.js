				...
				_attachBehavior: function () {
					var a = this,
						c = function (d) {
							...
							!a.options.disabled && a.options.clickCallback && a.options.clickCallback.call(this, d);
							b(a.$element).off("mouseup pointerup", c);
							a.pointerHandled = !1;
							if (d.type == "pointerup") a.completelyHandled = !0
						};
					...
					this.$element.on("keydown", function (b) {
						...
						a.options.disabled || a.$element.addClass(a.options.hoverClass + (a.mouseDown ?
							" " + a.options.activeClass : ""))
					}).on("mouseleave", function () {
						a.$element.removeClass(a.options.hoverClass + " " + a.options.activeClass);
						b(a.$element).off("mouseup", c)
					}).on("mousedown", function () {
						!a.options.disabled && !a.pointerHandled && (a.$element.addClass(a.options.activeClass), b(a.$element).on("mouseup", c))
					}).on("pointerdown", function () {
						if (!a.options.disabled) a.pointerHandled = !0, a.$element.addClass(a.options.activeClass), b(a.$element).on("pointerup", c)
					}).on("touchend", function (b) {
						if (a.completelyHandled) b.preventDefault(),
							a.completelyHandled = !1
					});
	...
	(function (b, c) {
		c.widget("Widget.RadioGroup", c.Widget, {
			...
			_bpDeactivate: function () {
				if (-1 != this.activeIndex) {
					var a =
						this._getElement(this.activeIndex);
			...
			_handlePrev: function () {
				if (!this.options.disabled) {
					if (this.activeIndex >
						this._getElementIndex(this.firstButton.$element[0])) this.activeIndex--;
			...
			checkButton: function (a) {
				...
				a !== c ? (c && this.uncheckButton(c),
					a && b(a).addClass(d)) : this.options.toggleStateEnabled && a && (this.uncheckButton(a, d), a = null);
			...
			disabled: function (a) {
				if (typeof a === "boolean") this.disabled = a, b.each(this.buttons, function () {
					this.disabled(a)
				});
				return this.options.disabled
			}
		...
		c.widget("Widget.TabGroup",
			c.Widget.RadioGroup, {
				...
				configureAria: function () {
					...
					else if (this.numButtons > 1) this.parentElement = this.buttons[0].$element.parents(this.options.parentSelectors.join()), this.parentElement.attr("role", "tablist"),
	...
	(function (b, c) {
		c.widget("Widget.PanelGroup", c.Widget, {
			...
			showPanel: function (a) {
				if (!this.showLock) {
					...
					if (c)
						...
						} else this.options.toggleStateEnabled &&
							this.hidePanel(c);
					--this.showLock
				}
			},
			...
			addTabGroup: function (a) {
				if (a)
					for (var a =
							c.ensureArray(a), d = a.length, j = 0; j < d; j++) {
						var f = a[j];
	...
	(function (b, c) {
		...
		c.Widget.Disclosure.AccordionTransitionPlugin = {
			...
			_attachBehavior: function (a) {
				...
				if (l && l.length > 0) a.options.marginBottom =
					Muse.Utils.getCSSIntValue(l, "margin-bottom"), a.options.originalHeight = l[0].scrollHeight;
			...
			_updateMarginBottomForRotatedAccordion: function (a) {
				a.options.rotatedAccordion.css("margin-bottom",
					Math.round(a.options.marginBottom - (a.options.rotatedAccordion[0].scrollHeight - a.options.originalHeight)) + "px")
			},
		...
		c.widget("Widget.SlideShowBase", c.Widget, {
			...
			stop: function () {
				...
				e.isDefaultPrevented() ||
					(this._stopTimer(), this.trigger("wp-slideshow-stop"))
			},
			isPlaying: function () {
			...
			_executeCall: function (a, b) {
				...
				e.isDefaultPrevented() || (this["_" + a].apply(this, b) && this.stop(), this.isPlaying() &&
					this._startTimer(!0), this.trigger("wp-slideshow-" + a))
			},
			first: function () {
	...
	(function (b, c) {
		c.widget("Widget.ContentSlideShow", c.Widget.SlideShowBase, {
			...
			_reprioritizeImageLoadingIfRequired: function (a) {
				!this._isLoaded(a) && this._cssilLoader &&
					!this._cssilLoader.isQueueEmpty() && (a = b(this.slides.$element[a]), this._cssilLoader.reprioritize(this._getAjaxSrcForImage(a.is("img") ? a : a.find("img")), this.isPlaying()))
			},
			_bpActivate: function () {
		...
		c.widget("Widget.Form", c.Widget, {
			...
			_submitFormInternal: function () {
				...
				b.ajax({
					...
					complete: function (k) {
						k && (k.status >= 400 || k.responseText && k.responseText.indexOf("<?php") >= 0) && alert("Form PHP script is missing from web server, or PHP is not configured correctly on your web hosting provider. Check if the form PHP script has been uploaded correctly, then contact your hosting provider about PHP configuration.");
						c.$element.removeClass(j);
						var i = null;
						if (k && k.responseText) try {
							i = d.parseJSON(k.responseText),
								i = i.FormProcessV2Response || i.FormResponse || i.MusePHPFormResponse || i
						} catch (n) {}
						if (i && i.success) {
							...
							c.$element[0].reset();
							c.hasCAPTCHA && c.$element.find("input:not([type=submit]), textarea").each(function () {
						...
						} else if (k = c._getFieldsWithError(i))
							for (i = 0; i < k.length; i++) c._switchState("invalid",
								k[i]);
						else c.$element.addClass(h);
						c.hasCAPTCHA || l.removeAttr("disabled");
						c.hasReCAPTCHA && Recaptcha.reload();
						c.hasReCAPTCHA2 && ("undefined" != typeof reCaptchaV2Manager ? reCaptchaV2Manager.reloadControls() : "undefined" != typeof grecaptcha && "undefined" != typeof muReCAPTCHA2Instances && b("[data-type=recaptcha2]", c.$element).each(function () {
			...
			_switchState: function (a, c) {
				...
				c.removeClass(d + " " + i + " " + l);
				if (a == "required" || a == "invalid") return a == "invalid" ? c.addClass(i) : c.addClass(l), "recaptcha" != b && this.errorSensitivity != "low" && (d = this.errorSensitivity == "high" ? "keyup" : "blur", c.data("error-state") || (c.data("error-state", !0), c.find("input, textarea").bind(d, m))), !1;
		...
		if (-1 != a) {
			...
			for (var a = document.getElementsByTagName("meta"), b = 0, c = a.length; b < c; b++) {
				...
				if ("generator" == d.getAttribute("name")) {
					"2017.0.2.363" != d.getAttribute("content") && Muse.assets.outOfDate.push("webpro.js");
					break
				}
			}
		}
